...
echo "Transferring 10 ETH from ganache account[0] to all others..."
...
for ((i = 1; i < signersLength; i++)); do
  ...
  txReceipt=$(curl $rootChainWeb3 -X POST --data '{"jsonrpc":"2.0","method":"eth_sendTransaction","params":[{"to":"'"$to_address"'","from":"'"$from_address"'","value":"0x8AC7230489E80000"}],"id":1}' -H "Content-Type: application/json")
